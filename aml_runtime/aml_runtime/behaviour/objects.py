"""Behaviour, role, scenario and lifecycle objects.

A behaviour object is a *hypothesis about what an account is doing over time*.
It is not a label: it carries the observations that support it, the observations
that argue against it, the time interval it covers, and the semantic objects it
was built from.  Counter-evidence is mandatory — a hypothesis that cannot state
what would weaken it is an assertion, not a hypothesis.
"""
...
class TimeInterval(Serializable):
    """The window a behavioural claim covers, on the stream's logical clock."""
...
class BehaviourObject(Serializable):
    """One behavioural hypothesis about one account over one interval."""
...
class BehaviourFact(Serializable):
    """A predicate over one behaviour object.

    Shaped like ``SemanticFact`` so the audit chain reads uniformly, but typed
    over the behaviour vocabulary rather than the semantic one.
    """
...
class RoleTransition(Serializable):
    """A causal record of an account changing what it is doing."""
...
class RoleObject(Serializable):
    """The account's current role, how long it has held it, and how it got there."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"id": self.id, "subject_id": self.subject_id, "role": self.role.value,
                ...
                "previous_role": self.previous_role.value, "explanation": self.explanation,
                "version": self.version}


@dataclass(frozen=True)
class ScenarioObject(Serializable):
    """An ordered story matched against the account's recent stage history."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"id": self.id, "type": self.type.value, "subject_id": self.subject_id,
                ...
                "supporting_behaviours": list(self.supporting_behaviours),
                "counter_evidence": list(self.counter_evidence),
                "causal_explanation": self.causal_explanation, "version": self.version}


@dataclass(frozen=True)
class LifecycleObject(Serializable):
    """Where the account is in its own observed life, independent of risk."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"id": self.id, "subject_id": self.subject_id, "age_minutes": self.age_minutes,
                ...
                "buckets_active": self.buckets_active, "first_seen_minute": self.first_seen_minute,
                "horizons_filled": list(self.horizons_filled),
                "horizons_unfillable": list(self.horizons_unfillable)}


@dataclass(frozen=True)
class BehaviourReading(Serializable):
    """Everything the Behaviour Layer has to say about one event."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "behaviours": [item.to_dict() for item in self.behaviours],
